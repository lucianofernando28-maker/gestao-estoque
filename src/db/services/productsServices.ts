...
export const productsServices = {
  ...
  async createProduct(data: {name:string,price:number,stockQuantity:number}) {
    try {
      const totalPrice =data.price * data.stockQuantity;   return await db.insert(products).values({...data,totalPrice}).returning();
    } catch (error) {
